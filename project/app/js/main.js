...
setTimeout(()=>{
  const deleteEventButton = document.getElementsByClassName('delete-event-button');
  if (deleteEventButton) {
    console.log(deleteEventButton)
    for (var item in deleteEventButton) {
      if(typeof deleteEventButton[item] !=="object") return
        deleteEventButton[item].addEventListener('click', deleteAndPostEvent);
    }
  }
},200)
...
function deleteAndPostEvent(e) {
  e.preventDefault();

  data = {id:e.srcElement.name}
  const body = JSON.stringify(data);
  deleteEventDataLocally(e.srcElement.name)
  const headers = new Headers({'Content-Type': 'application/json'});
    ...
    const item =
      `<li class="card">
         <div class="card-text">
           ...
           <h4>${event.city}</h4>
           <p>${event.note}</p>
           <button name="${event.id}" type="danger" class="delete-event-button danger button ripple">
             delete
           </button>
         </div>
       </li>`;
...
function deleteEventDataLocally(id) {
  ...
  .then(offlineData => {
    if (!offlineData.length) {
      ...
      // messageOffline();
      console.log(offlineData,id)
      offlineData.forEach(item =>{
        if (item.id == id) {
          offlineData.splice(item, 1)
          var ddd= item
          return dbPromise.then(db => {
            console.log(ddd)
            const tx = db.transaction('events', 'readwrite');
